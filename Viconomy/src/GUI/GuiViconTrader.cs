...
namespace Viconomy.GUI
{
    ...
    public class GuiViconTrader : GuiDialog
    {
        public override string ToggleKeyCombinationCode => null;

        InventoryBase traderInventory;
        ...
        public GuiViconTrader(InventoryBase traderInventory, EntityAgent owningEntity, ICoreClientAPI capi, int rows = 4, int cols = 4) : base(capi)
        {
        ...
        void CalcAndUpdateAssetsDisplay()
        {
            ...
            SingleComposer.GetDynamicText("traderMoneyText")?.SetNewText(Lang.Get("{0} has {1} Gears", owningEntity.GetBehavior<EntityBehaviorNameTag>().DisplayName, 0));
        }
        ...
        private void TraderInventory_SlotModified(int slotid)
        {
           ...
           // int totalGain = traderInventory.GetTotalGain();
            ...
            SingleComposer.GetDynamicText("costText")?.SetNewText(0 > 0 ? Lang.Get("Total Cost: {0} Gears", 0) : "");
            SingleComposer.GetDynamicText("gainText")?.SetNewText(0 > 0 ? Lang.Get("Total Gain: {0} Gears", 0) : "");

            if (SingleComposer.GetButton("buysellButton") != null)
            {
                SingleComposer.GetButton("buysellButton").Enabled = 0 > 0 || 0 > 0;
        ...
        {
            EnumTransactionResult result = EnumTransactionResult.Success; //traderInventory.TryBuySell(capi.World.Player);
