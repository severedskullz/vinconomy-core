...
{
    public class BEVRegisterLegacy : BlockEntityContainer
    {
        ...
        public BEVRegisterLegacy()
        {
            inventory = new InventoryGeneric(25, null, null);
        }
        ...
        public override void Initialize(ICoreAPI api)
        {
            ...
            //block = (BlockVRegister)api.World.BlockAccessor.GetBlock(Pos);
        ...
        public void UpdateShop(string Owner, string OwnerName, int ID, string Name)
        {
            if (Api.Side == EnumAppSide.Server)
            {
                if (ID <= 0)
                {
                    ...
                    ShopRegistration register = modSystem.AddShop(Owner, OwnerName, OwnerName + "'s Shop", Pos);
                ...
                else
                {
                    ...
                    if (reg != null)
                    {
                        ...
                        ShopRegistration register = modSystem.UpdateShop(Owner, ID, Name, Pos);
                    }
                    else
                    {
                        modSystem.Mod.Logger.Warning("Somehow the shop with ID " + ID + " got removed. Recreating it...");
                        ShopRegistration register = modSystem.AddShop(Owner, OwnerName, Name, Pos, ID);
        ...
        private void OpenAdminForPlayer(IPlayer byPlayer)
        {
            if (Api.World is IServerWorldAccessor)
            {
              ...
              ((ICoreServerAPI)Api).Network.SendBlockEntityPacket((IServerPlayer)byPlayer, Pos.X, Pos.Y, Pos.Z, VinConstants.TOGGLE_GUI, data);
        ...
        private void OpenShopGui(byte[] data)
        {
            ...
            invDialog = new GuiViconRegister(dialogTitle, Inventory, Pos, Api as ICoreClientAPI);
            waypointDialogue = new GuiViconWaypoint("Waypoint", Pos, ID, Api as ICoreClientAPI);
            ...
            invDialog.OnClosed += delegate ()
            {
                ...
                ((ClientCoreAPI)Api).Network.SendBlockEntityPacket(Pos.X, Pos.Y, Pos.Z, VinConstants.CLOSE_GUI, null);
            };
            waypointDialogue.TryOpen();
