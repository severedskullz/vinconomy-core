...
{
    public class VinconMealInventory : VinconBaseInventory, IStallSlotUpdater
    {
        public VinconItemSlot TransferSlot;
        public override int Count => (StallSlotSize * NumStalls) + 2;
        public VinconMealInventory(BEVinconBase stall, string inventoryID, ICoreAPI api, int numStalls, int numStacksPerStall) : base(stall, inventoryID, api, numStalls, numStacksPerStall)
        {
            TransferSlot = new VinconItemSlot(this, -1, 1);
            TransferSlot.SetFilter(VinconomyFilters.IsFoodContainer);
        ...
        public override ItemSlot this[int inventorySlotId]
        {
            ...
            set
            {
                ...
                {
                    TransferSlot = (VinconItemSlot)value;
                }
                else
                {
                    if (inventorySlotId-2 < 0 || inventorySlotId >= Count)
