...
{
    public class VinconGachaInventory : InventoryBase, ISlotProvider, IStallSlotUpdater
    {
        ...
        public VinconGachaInventory(int numSlots, string inventoryID, ICoreAPI api) : base(inventoryID, api)
        {
        ...
        protected override ItemSlot NewSlot(int id)
        {
            ...
            {
                VinconItemSlot slot = new VinconItemSlot(this, 0, id);
                slot.SetFilter(VinconomyFilters.IsFilledGachaSlot);
        ...
        public VinconItemSlot GetRandomItem(bool isPickAbsolute)
        {
            VinconItemSlot[] filled = GetNonEmptySlots();
            if (filled.Length == 0)
            {
                return null;
            }
        ...
        private VinconItemSlot[] GetNonEmptySlots()
        {
            List<VinconItemSlot> filledSlots = new List<VinconItemSlot>();
            for (int i = 1; i < slots.Length; i++)
            {
                if (slots[i].Itemstack != null)
                {
                    filledSlots.Add((VinconItemSlot)slots[i]);
                }
            }
        ...
        {
            return (VinconCurrencySlot)slots[0]; // Always return the currency slot for all "Stalls"
        }
